...
func fdctIJG(b *block) {
	...
	for i := range 8 {
		...
		b[0*8+i] = (tmp10 + tmp11) >> (pass1Bits-pass2Bits)
		b[4*8+i] = (tmp10 - tmp11) >> (pass1Bits-pass2Bits)

		z1 := (tmp12 + tmp13) * fix_0_541196100
		z1 += 1 << (constBits + pass1Bits - 1)
		b[2*8+i] = (z1 + tmp12*fix_0_765366865) >> (constBits + pass1Bits-pass2Bits)
		b[6*8+i] = (z1 - tmp13*fix_1_847759065) >> (constBits + pass1Bits-pass2Bits)
		...
		tmp12 += z1
		tmp13 += z1
		b[1*8+i] = (tmp0 + tmp10 + tmp12) >> (constBits + pass1Bits-pass2Bits)
		b[3*8+i] = (tmp1 + tmp11 + tmp13) >> (constBits + pass1Bits-pass2Bits)
		b[5*8+i] = (tmp2 + tmp11 + tmp12) >> (constBits + pass1Bits-pass2Bits)
		b[7*8+i] = (tmp3 + tmp10 + tmp13) >> (constBits + pass1Bits-pass2Bits)
	}
}
