...
#ifndef Just_16
/* When Pack_32 is not defined, we store 16 bits per 32-bit int.
 * This makes some inner loops simpler and sometimes saves work
 * during multiplications, but it often seems to make things slightly
 * slower.  Hence the default is now to store 32 bits per int.
 */
#ifndef Pack_32
#define Pack_32
#endif
...
s2b
#ifdef KR_headers
	(s, nd0, nd, y9) CONST char *s; int nd0, nd; unsigned int y9;
#else
	(CONST char *s, int nd0, int nd, unsigned int y9)
#endif
{
	Bigint *b;
	int i, k;
...
;
	...
	for(result_k = 0; sizeof(Bigint) - sizeof(unsigned int) + j < i;
		j <<= 1) result_k++;
