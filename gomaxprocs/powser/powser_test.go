...
func BenchmarkPowser(b *testing.B) {
	if once {
		b.Fatal("can only run once")
	}
	if b.N != 1 {
		b.Fatal("can only use N=1")
	}
	once = true

	for i := 0; i < 4000; i++ {
		drain(Integ(zero, MonSubst(Ones, neg(one), 2)), 500)
	}
}
...
// Drain n terms of a power series
func drain(U PS, n int) {
	done := false
	for ; !done && n > 0; n-- {
		u := get(U)
		if end(u) != 0 {
