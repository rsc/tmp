...
func casesz(ctxt *liblink.Link, p *liblink.Prog) int32 {
...
func asmoutnacl(ctxt *liblink.Link, origPC int32, p *liblink.Prog, o *Optab, out []uint32) int {
 ...
 */
func checkpool(ctxt *liblink.Link, p *liblink.Prog, sz int) int {
...
func flushpool(ctxt *liblink.Link, p *liblink.Prog, skip int, force int) int {
	var q *liblink.Prog
...
func regoff(ctxt *liblink.Link, a *liblink.Addr) int32 {
	ctxt.Instoffset = 0
	aclass(ctxt, a)
...
func oplook(ctxt *liblink.Link, p *liblink.Prog) *Optab {
	...
	ctxt.Diag("illegal combination %P; %^ %^ %^, %d %d", p, a1, a2, a3, p.From.Type_, p.To.Type_)
	...
	case 11: /* word */
		...
		if p.To.Sym != nil {
			...
			if rel.Sym == ctxt.Tlsg {
				...
				rel.Type_ = liblink.R_PCREL
				rel.Add += ctxt.Pc - p.Pcrel.Pc - 8
			} else {

				rel.Type_ = liblink.R_ADDR
			}
			o1 = 0
		}
...
func olr(ctxt *liblink.Link, v int32, b int, r int, sc int) uint32 {
	var o uint32
...
func olhr(ctxt *liblink.Link, v int32, b int, r int, sc int) uint32 {
	var o uint32
...
func osr(ctxt *liblink.Link, a int, r int, v int32, b int, sc int) uint32 {
	var o uint32
...
func oshr(ctxt *liblink.Link, r int, v int32, b int, sc int) uint32 {
	var o uint32
...
func ofsr(ctxt *liblink.Link, a int, r int, v int32, b int, sc int, p *liblink.Prog) uint32 {
	var o uint32
