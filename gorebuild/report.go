	...
	for _, arg := range args {
		...
		for _, version := range versions {
			...
			for _, f := range rel.dl.Files {
				if sys == "" || sys == f.GOOS+"-"+f.GOARCH {
					r.File(rel, f.Name, f.GOOS, f.GOARCH, f)
					// TODO add module files too
				}
			}
		}
	}
...
func (r *Report) ReproFile(rel *Release, file *File, src []byte) (err error) {
	...
	for _, b := range built {
		...
		bf := r.File(rel, name, file.GOOS, file.GOARCH, nil)
...
func (r *Report) ReproWindowsMsi(rel *Release, file *File, zip []byte) {
	mf := r.File(rel, strings.TrimSuffix(file.Name, ".zip")+".msi", file.GOOS, file.GOARCH, nil)
	if mf.dl == nil {
...
func (r *Report) ReproDarwinPkg(rel *Release, file *File, tgz []byte) {
	pf := r.File(rel, strings.TrimSuffix(file.Name, ".tar.gz")+".pkg", file.GOOS, file.GOARCH, nil)
	if pf.dl == nil {
...
func (r *Report) File(rel *Release, name, goos, goarch string, dl *DLFile) *File {
	rel.mu.Lock()
	defer rel.mu.Unlock()
